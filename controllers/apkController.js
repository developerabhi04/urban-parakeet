...
export const uploadApk = async (req, res) => {
    try {
        if (!req.file) return res.status(400).json({ success: false, error: 'APK file required' });
        ...
        res.status(201).json({ success: true, data: apk, message: 'APK uploaded' });
    } catch (e) {
...
export const getApkStatus = async (_req, res) => {
    ...
    res.json({ success: true, available: !!apk, fileInfo: apk });
};

export const downloadApk = async (_req, res) => {
    const apk = await Apk.findOne();
    if (!apk) return res.status(404).json({ success: false, error: 'APK not found' });
