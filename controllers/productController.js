...
const getTopDeals = async (req, res) => {
    try {
        ...
        const minDiscount = parseInt(req.query.minDiscount) || 5;  // Lowered threshold
        const minDiscountAmount = parseInt(req.query.minDiscountAmount) || 10; // Lowered threshold
        ...
        if (totalProducts === 0) {
            // Auto-insert sample data if database is empty
        ...
        const products = await Product.find({
            ...
            .sort({
                discountPercent: -1,  // Sort by discount percentage (highest first)
                discount: -1,         // Then by discount amount (highest first)
