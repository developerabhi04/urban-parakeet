    ...
    try {
        ...
        if (req.files && req.files.length > 0) {
            newImages = req.files.map(file => ({
                url: file.path,
                public_id: file.filename,
                alt: `${req.body.name || product.name} - Image`
            }));
        }
        ...
        if (updateData.dmartPrice) updateData.dmartPrice = parseFloat(updateData.dmartPrice);
        ...
        if (updateData.reviewsCount !== undefined) {
            const newReviewsCount = parseInt(updateData.reviewsCount);
            ...
            updateData.reviewsCount = newReviewsCount;
        }
...
const updateProductRating = async (req, res) => {
    try {
        ...
        const updateData = {};
        if (rating !== undefined) updateData.rating = Math.round(parseFloat(rating) * 10) / 10;
